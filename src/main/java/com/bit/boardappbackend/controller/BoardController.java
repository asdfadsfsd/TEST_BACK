        ...
        try {
            if(uploadFiles != null && uploadFiles.length > 0) {
                Arrays.stream(uploadFiles).forEach(file -> {
                    log.info("upload files: {}", file.getOriginalFilename());
                });
            }
            if(changeFiles != null && changeFiles.length > 0) {
                Arrays.stream(changeFiles).forEach(file -> {
                    log.info("change files: {}", file.getOriginalFilename());
                });
            }

            BoardDto modifyBoardDto = boardService.modify(boardDto, uploadFiles,changeFiles,originFiles,customUserDetails.getMember());
